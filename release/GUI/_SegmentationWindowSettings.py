...
class Settings:
    ...
    def reset_visibility_VAD(self):
        ...
        self.grpSettVADManualOpt.setVisible(False)
    ...
    def do_VAD(self,apply=False):
        ...
        if self.VADmethod != 'None':
            ...
            #----------------------------------------------------------------
            #----------------------------------------------------------------
            #Save the segment manually segmented
            #----------------------------------------------------------------
            #----------------------------------------------------------------
            if self.VADmethod == 'Manual':
                #Activate manual segmentation options
                self.box_properties(self.grpSettVADManualOpt)
                if apply:
                    if len(self.voiceSegs.values())==0:     
                        DialogsWindow.ErrorDiagWin('Select the segments from the figure and add them by clicking the add button')
                        self.ResetSegments()
                    # else:
            ...
            #----------------------------------------------------------------
            #----------------------------------------------------------------
            elif self.VADmethod == 'Energy':
                #Delete any segment and recompute        
                self.ResetSegments()
                ...
                if apply:
                    ...
                    #-
                    times,self.VAD_Ethr = self.VADMeth['Energy'](sig,fs,win=win/1000,step=step/1000,VAD_thr=self.pltSignal.setThresh)
                    if len(times)!=0:
    ...
    def do_filter(self,apply=False,fplot=True):
        #Reset visibility
